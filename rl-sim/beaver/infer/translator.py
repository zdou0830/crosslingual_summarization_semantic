...
from beaver.infer.beam import Beam, Greedy
...
def greedy_search(opt, model, src, fields, flag):
    batch_size = src.size(0)
    beam_size = opt.beam_size
    device = src.device
    ...
    beams = [Greedy(1, tgt_field.pad_id, tgt_field.bos_id, tgt_field.eos_id,
                  device, opt.length_penalty) for _ in range(batch_size)]
    ...
    for i in range(opt.max_length):
        ...
        # find topk candidates
        scores, indexes = (out).view(batch_size, -1).topk(1)
        tokens = indexes.view(batch_size, 1)
...
def sampling(opt, model, src, fields, flag, tempe=0.15):
    batch_size = src.size(0)
    device = src.device
    ...
    for i in range(opt.max_length):
        ...
        out = torch.div(out, tempe)

        # find topk candidates
        dist = torch.distributions.Multinomial(logits=out, total_count=1)
        indexes = torch.argmax(dist.sample(), dim=-1, keepdim=True)
        tokens = indexes.view(batch_size, 1)
