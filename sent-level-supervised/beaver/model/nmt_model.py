...
class NMTModel(nn.Module):
    ...
    def forward(self, source, target, flag, pos=None):
        ...
        if flag:  # task1
            ...
            if pos is not None:
              src_cls = source.eq(self.cls_id1).unsqueeze(-1)
              ext_in = torch.masked_select(enc_out, src_cls)
              ext_in = ext_in.view(-1, enc_out.size(-1))
            ...
            if pos is not None:
              src_cls = source.eq(self.cls_id2).unsqueeze(-1)
              ext_in = torch.masked_select(enc_out, src_cls)
              ext_in = ext_in.view(-1, enc_out.size(-1))
              ext_scores = self.task1_extractor(ext_in)
              return scores, ext_scores
